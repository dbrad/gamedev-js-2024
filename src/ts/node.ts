...
let null_rel_fn = (node_id: number, x: number, y: number, delta: number): void => { };
type NodeFn = (node_id: number) => void;
let null_node_fn = (node_id: number): void => { };
...
export let new_node = (parent_id: number = -1, update_fn: RealtiveFn = null_rel_fn, draw_fn: RealtiveFn = null_rel_fn, reset_fn: NodeFn = null_node_fn): number =>
{
    let id = next_id++;
...
export let reset_node = (node_id: number): void =>
{
...
export let update_node = (node_id: number, x: number, y: number, delta: number): void =>
{
...
export let draw_node = (node_id: number, x: number, y: number, delta: number): void =>
{
