...
export function create_splash_screen(root_id: number): number
{
...
function update_splash_screen(node_id: number, x: number, y: number, delta: number): void
{
    if (A_PRESSED)
    {
...
function draw_splash_screen(node_id: number, x: number, y: number, delta: number): void
{
